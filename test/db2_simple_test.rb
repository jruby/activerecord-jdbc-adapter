...
class DB2Test < Test::Unit::TestCase
  ...
  def test_select_schema
    ...
    @config[:url] = 'jdbc:as400://localhost/schema;naming=system;libraries=lib1,lib2'
    assert_equal 'schema', @adapter.send(:db2_schema)
    
    @config[:url] = 'jdbc:as400://127.0.0.1/SCHEMA1 '
    assert_equal 'SCHEMA1', @adapter.send(:db2_schema)
    
    @config[:url] = 'jdbc:as400://127.0.0.1/SCHEMA2;'
    assert_equal 'SCHEMA2', @adapter.send(:db2_schema)

    @config.delete(:url)
    @config[:jndi] = 'java:comp/env/DB2DS'
    assert_equal nil, @adapter.send(:db2_schema)
    
    @config.delete(:jndi)
    @config[:host] = 'localhost'
    @config[:username] = 'db2inst1'
    assert_equal 'db2inst1', @adapter.send(:db2_schema)
  end
